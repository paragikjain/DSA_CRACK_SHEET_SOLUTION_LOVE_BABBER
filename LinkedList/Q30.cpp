...
//step 1: copy each node of link list and place between curr node and next node 
 Node *curr =head;
 while(curr!=NULL){
      Node *next=curr->next;
 ...
 //step 2 with help of older random connection create same random connection
...
//step 3 Remove old connection so extract new nodes from link list
  Node *org=head;
  Node *New=head->next;
  Node *head_new=New;
  Node *test=head;
  while(org!=NULL && New!=NULL ){
      Node *tmp=org->next->next;
      org->next=tmp;
      if(tmp)
      New->next=tmp->next;
      else
